...
func TestMovieHash_Deterministic(t *testing.T) {
	...
	allure.Test(t, "moviehash is stable for a fixed file", func(a *allure.Context) {
		...
		hash1, size1, err := mediahash.MovieHash(path)
		if err != nil {
			t.Fatal(err)
		}
		hash2, size2, err := mediahash.MovieHash(path)
		if err != nil {
			t.Fatal(err)
		}
...
func TestMovieHash_TooSmall(t *testing.T) {
	t.Parallel()

	allure.Test(t, "moviehash rejects files under 64KiB", func(a *allure.Context) {
		t := a.T()
		path := filepath.Join(t.TempDir(), "tiny.bin")
		err := os.WriteFile(path, []byte("tiny"), 0o600)
		if err != nil {
			t.Fatal(err)
		}

		_, size, err := mediahash.MovieHash(path)
		if err == nil {
			t.Fatal("expected ErrFileTooSmall")
		}
		if size != 4 {
			t.Fatalf("size: got %d want 4", size)
		}
	})
}
